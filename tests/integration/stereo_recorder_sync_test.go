...
func TestStereoRecording_StaysAlignedAcrossIncomingStall(t *testing.T) {
	...
	// The recorder's own clock keeps the file advancing through the stall, so the
...
// TestStereoRecording_SurvivesOutgoingStall is the mirror case, and the one a
// producer-paced recorder cannot handle. Holding the *recorded* leg stops its
// writeLoop from writing the outgoing tap at all — as an outbound DTMF burst and
// a deafened room participant also do — while incoming RTP keeps decoding into
// the incoming tap. A recorder clocked by the outgoing tap would freeze for the
// whole hold and lose it from the timeline; clocked by itself, the file stays in
// real time and the incoming audio keeps its true offsets.
func TestStereoRecording_SurvivesOutgoingStall(t *testing.T) {
	instA := newTestInstance(t, "instance-a")
	instB := newTestInstance(t, "instance-b")
	...
	const stallFrames = 40 // ~800 ms with no outgoing tap writes at all
	...
	// Hold the recorded leg: its writeLoop skips the outgoing tap entirely, so
	// the right channel's producer goes completely silent. The read path is not
	// gated on hold, so incoming audio keeps arriving throughout.
	legA.SetHeld(true)
	feed(stallFrames, false)
	legA.SetHeld(false)
	...
	// The headline assertion: the recording still spans the call. A recorder
	// paced by the outgoing tap would come back short by the whole hold — about
	// 800 ms of a ~2.9 s call — which lands far below this bound.
	wantSamples := int(elapsed.Seconds() * stallCallRate)
	minSamples := wantSamples * 85 / 100
	t.Logf("recorded %d samples/channel over a %v call (%.0f%%), hold was %v",
		len(right), elapsed.Round(time.Millisecond),
		100*float64(len(right))/float64(wantSamples),
		time.Duration(stallFrames)*stallFrameTime)
	if len(right) < minSamples {
		t.Errorf("recorded %d samples/channel, want >= %d (85%% of the %v call): "+
			"the outgoing stall froze the recording instead of silence-filling it",
			len(right), minSamples, elapsed.Round(time.Millisecond))
	}

	// Incoming audio never stalled, so both markers must still be on the left
	// channel — the hold must not have cost the recorder any of it.
	const debounce = 10 * (stallFrameLen / 2) // 200 ms
	onsetsL := burstOnsets(left, stallBurstFloor, debounce)
	if len(onsetsL) != 2 {
		t.Fatalf("left channel has %d marker bursts at %v, want 2 (incoming audio lost across the outgoing stall)",
			len(onsetsL), onsetsL)
	}

	// Marker 2 sits after the hold, so its position pins that the hold occupies
	// real time in the file rather than being collapsed out of it.
	stallSamples := int((time.Duration(stallFrames) * stallFrameTime).Seconds() * stallCallRate)
	gap := onsetsL[1] - onsetsL[0]
	if minGap := stallSamples; gap < minGap {
		t.Errorf("left markers are %d samples apart, want >= %d: the hold was collapsed out of the timeline",
			gap, minGap)
	}
}
