...
func configVars() *seq {
	vars := []configVar{
		...
		{Name: "SIP_MULTI_STREAM_ENABLED", Default: "false", Description: "Allow a SIP dialog to negotiate more than one m=audio section (RFC 3264 §5.1), so a call can carry several independent audio streams — e.g. original plus translated audio. When false, extra audio sections offered by a peer are answered with port 0 and the per-leg stream API refuses to add any. Off by default because many endpoints and SBCs assume exactly one m=audio line."},
		{Name: "SIP_MULTI_STREAM_MAX", Default: "4", Description: "Maximum audio sections per SIP dialog when SIP_MULTI_STREAM_ENABLED is true. Each stream consumes one port from the RTP_PORT_MIN/RTP_PORT_MAX pool. Clamped to 1..16."},
