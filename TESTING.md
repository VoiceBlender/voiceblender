...
| `internal/sip` (whatsapp) | 19 | `IsWhatsAppInvite` requires Meta From host **and** DTLS fingerprint (exact/subdomain/lookalike/case-insensitive; plain RTP after a terminating proxy does not match), `WhatsAppRecipientURI` E.164 normalisation, `InviteWhatsApp` precondition checks (TLS configured, required fields) |
