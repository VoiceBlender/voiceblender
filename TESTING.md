...
| `TestRoomWSJitterBufferAbsorbsIngressStalls` | Runs the same periodically-stalling WS leg producer with `WS_JITTER_BUFFER_MS` at `0` and at `400`, and compares `Mixer.ParticipantFeed`'s starved-tick count over a 3 s steady-state window. Passthrough must starve on the stalls; the playout lead must absorb at least half of them — regression guard for the lead never being rebuilt after an underrun |
