...
| `internal/mixer` | 42 | Audio mixing, configurable sample rate (8/16/48 kHz), anti-aliasing polyphase resampler (stopband attenuation, passband flatness, group-delay budget, seam continuity, full-scale clamp, `Close` propagation through the wrapper), playback sources, tap recording, per-listener routing whitelist (full mesh / supervisor-whisper / isolated / mute+deaf interaction), and panic containment: an IO-loop panic removes only its own participant instance (a stale loop cannot evict its successor), the owner hook fires exactly once, the writer is closed to wake a ws/agent owner but spared when the owner closes its own egress, and a `mixTick` panic skips one tick with rate-limited logging while the room keeps running; and `ResampleInto`, the allocation-free counterpart of `ResampleSamples` used by the audio filter chain's hot path — bit-identical output across rate pairs over repeated frames, destination buffer reused without reallocation, and nil-receiver passthrough |
...
| `internal/audiofilter` | 44 | Per-leg filter chain: framing independence (identical output across source chunk sizes 1-65536 and read sizes 1-4096, including odd sizes that split a sample and straddle a block), zero steady-state allocation on the `Read` path, working-rate resolution from filter requirements, block size as the LCM of stage frame lengths with sub-slice dispatch, empty-chain passthrough, `Close` propagation to stages and source, rate conversion while filtering; registry parse/validate (unknown name, malformed parameter, chain-length cap, single-use filters, duplicate registration); `bandpass` in-band retention vs out-of-band attenuation with state carried across frames, invalid bands rejected and above-Nyquist clamped; `gain` pinned to the playback volume curve (~3 dB per step over -8..8), clamping and range enforcement; `pitch` verified to shift by the requested interval to within ±15 cents across the full -12..+12 range while leaving the sample count untouched (a shifter that changed duration would drift the leg against the mixer clock), with an equal-power crossfade that does not clip a near-full-scale talker; `robotic` (a feedback comb, not a vocoder) verified to keep the original waveform — correlation with the input 0.66 against the vocoder's 0.06, the measurement that explains why one stays intelligible and the other does not — plus resonance at the configured pitch, stability at maximum depth over 10 s, `mix=0` passthrough and parameter rejection; `vocoder` verified as a real vocoder rather than a passthrough or ring modulator (an octave-safe autocorrelation check confirms two different input pitches both come out at the carrier's pitch), plus level tracking, no carrier bleed on silence, the limiter holding a near-full-scale talker below the clamp, analysis-bank passband ripple, and unvoiced input staying noise-like instead of being buzzed by a periodic carrier (periodicity 0.02 with the voiced/unvoiced crossfade, 0.85 without); and `Resolve` dropping a registered-but-unavailable filter so a chain naming it degrades to passthrough instead of failing call setup |
| `internal/audiofilter/denoise` | 19 | RNNoise, pure Go and no cgo: kernel Install/Available/Shutdown lifecycle and idempotent shutdown; degraded mode with no kernel (name still validates, `Resolve` drops it, chain builds as passthrough, explicit error if built anyway); steady-state suppression on band-limited noise (speech band and rumble) after the estimator converges; the ~1 s convergence ramp, which is operationally visible at the start of a call; full-band white noise held inside a measured window as a regression guard on the kernel and its wiring; frame-size rejection; state independence (two fresh states agree, recurrent history carries) and a recycled state reproducing a fresh one exactly, so one call's history cannot reach the next; concurrent drive across 16 states; the full chain at the 16 kHz room rate leaving the work rate alone; native operation at 8, 16 and 48 kHz (frame length following the rate, suppression measured at each, the stage reporting its own frame to the chain, a rate below the model's 8 kHz minimum rejected, and states pooled per rate so an 8 kHz state is never handed to a 16 kHz leg); and the case that motivates running voice activity detection after the chain rather than before — on noise-only input at three times the VAD threshold, 250 of 250 frames read as speech before denoising and 0 of 250 after — plus the chain observer delivering exactly the filtered output the reader produced; and the push-mode `Writer` that feeds answering-machine detection — which starts during ringing, before any room chain exists — denoising and resampling 8 kHz to AMD's 16 kHz, returning its kernel state on Close, and degrading to the plain resampling writer when the leg has no corrective filters |
...
| `internal/room` (audio filters) | 4 | Per-leg chain selection: a leg that chose nothing takes the server default, a leg that chose one keeps it, and an explicitly empty chain is *not* given the default back (the opt-out would otherwise be silently undone); and a chain that cannot be built falls back to plain resampling so the leg keeps its audio; and a leg that joined with no filters still gets a (transparent) chain, so denoise can be turned on and off mid-call for it — without that the room registered nothing and the change failed for exactly the legs most likely to want it; and a leg moved between rooms rebuilds its chain in the new room, hands the old chain's denoise state back to the pool, and keeps its own audio source open — the mixer closes a participant's reader on teardown, so a chain that propagated Close would leave a pipe-backed leg (a LiveKit participant) silent after the move |
| `internal/room` (comfort noise) | 5 | Server default reaches rooms created directly and by `MoveLeg`; the per-room `CreateOptions.ComfortNoise` override wins in both directions, nil keeps the default, and an override does not change the default for later rooms |
| `internal/room` (rate changes) | 5 | Rebuilding a leg's mixer participant when a mid-call renegotiation moves its sample rate: the participant is replaced (its resamplers were sized for the old rate), the chain that is *actually running* is carried over rather than the leg's configured one, and its observers with it — a dropped observer is a detector that never fires again on that call; the rate-change watch is installed on join and cleared on leave, so a room never rebuilds for a leg it lost; and a change is routed to the stream that moved, with the primary falling through to the leg participant |
| `internal/api` (audio filters) | 4 | Request conversion preserving the omitted/empty/populated distinction the room depends on, validation rejecting unknown filters and over-long chains, the leg view reporting the chain that actually runs (and omitting the field when nothing runs), and `Parse`/`Format` round-tripping the `AUDIO_FILTERS` syntax |
...
| `internal/metrics` | 12 | Prometheus collector: `/metrics` exposition, active-leg and active-room gauges, disconnect-reason and duration series from bus events, `voiceblender_recovered_panics_total` (registered on every collector, incremented off the panic-recovery sites), and the egress counters (`webhook_enqueued`/`webhook_dropped`/`webhook_deliveries{outcome}`/`vsi_events_dropped`) — registered at zero and incremented through the `events.MetricsObserver` methods); and the audio-filter signals — the `denoise` gauges reading the pool at scrape time (following an installed source, ignoring a nil one) and the `filter`-labelled drop counter, which carries no series until something is actually dropped |
| `internal/events` | 39 | Event bus (subscribe/publish/unsubscribe, timestamp stamping), flattened `MarshalJSON` envelope, per-leg `custom_data` (verbatim round-trip including large integers and key order, `null` marshalling, registry set/replace/clear + copy-on-store + concurrent access, stamped onto leg-scoped events at publish and absent from room-scoped ones, emitted as a top-level `custom_data` key and omitted when empty), DTMF/RTT per-leg sequence numbers, webhook registry (leg/room/global routing, HMAC signing, `Stop`); the stable `event_id` (unique per publish, identical across subscribers, omitted on unpublished events, constant across all 3 retry attempts and matching the `X-Event-Id` header); and the exactly-one-outcome invariant for `deliver` (`success`, `exhausted`, `marshal_error`, `request_error`) plus full-queue drop counting and nil-observer safety |
| `internal/speaking` | 7 | Voice activity detection, debouncing, mute handling, 8kHz/16kHz sample rates. Note the detector is attached *after* the leg's filter chain (see `internal/audiofilter/denoise`): it is RMS with hysteresis, so a raised noise floor alone trips it |
...
| `internal/sip` (whatsapp) | 22 | `IsWhatsAppInvite` requires Meta From host **and** DTLS fingerprint (exact/subdomain/lookalike/case-insensitive; plain RTP after a terminating proxy does not match; the attribute counts only at the start of an SDP line, and is found inside a multipart body), `IsMetaFromHost` matching the From host alone, `WhatsAppRecipientURI` E.164 normalisation, `InviteWhatsApp` precondition checks (TLS configured, required fields) |
...
| `internal/sip` (inbound auth) | 12 | `recordChallenge` WWW-Authenticate shape + nonce uniqueness; `VerifyInboundAuth` valid digest (MD5/SHA-256, password and HA1), wrong password/username → invalid, no/unknown/expired challenge → none, single-use nonce consumption, the `max_expires` TTL cap and `app_id` round-trip through the pending challenge |
| `internal/api` (inbound auth) | 14 | `HandleRegisterAttempt` always publishes the `sip.registration_attempt` event, scopes it to the binding's owning `app_id` when one exists, applies the configured fallback when undecided (reject by default, accept when configured), propagates a challenge decision (incl. `max_expires` and `app_id`), accept decision carries `max_expires` and `app_id`; `sharedAppID` resolving a registered socket's owner (agreeing, disagreeing, partly unclaimed); `registerConsultFallback` mapping (unset/unknown → reject, `accept` case-insensitive); `ChallengeRequest` validation (realm + password/ha1 required, non-negative `max_expires`) |
| `internal/api` (custom data) | 15 | `custom_data` plumbing: the `CUSTOM_DATA_MAX_BYTES` check (under/over/`0`=unlimited), omit-vs-`null`-vs-replace semantics of `applyCustomData`, the `custom_data` query-param parser (empty, valid, malformed JSON, oversize), rejection before the leg-type dispatch on `POST /v1/legs` so every type is covered, any-JSON-value acceptance, `LegView` carrying and omitting the field, `custom_data` surviving on `leg.disconnected` and being cleared afterwards, requests that fail a precondition storing nothing, the optional `/ring` body, and the `PUT`/`DELETE /v1/legs/{id}/custom-data` endpoints (replace, `null`-clears, required-field and oversize rejection, idempotent delete, mid-call change visible on the next event) |
...
| `internal/sip` (trunk proxy) | 10 | `PeerSocket` keying on the proxy when one is set (registrar and proxy hosts deliberately differ so a wrong reach is observable) and on the registrar when not; `nextHopURI` and `nextHopSocket` (explicit port, `sip:`/`sips:` defaults, empty when unconfigured); `buildRegister` steering the REGISTER at the proxy socket with **no** pre-loaded Route while the Request-URI stays the registrar — a proxy that does not recognise the Route URI as its own forwards the REGISTER back at itself and the registration dies in silence — proven end to end by a registrar name that resolves nowhere, so the request can only arrive if the proxy socket is the destination; `buildDigestResponse` computing against the registrar and never the proxy (RFC 3261 §22.4); `Snapshot` reporting the effective hop and omitting it when unset |
| `internal/sip` (trunk inbound match) | 10 | `LookupInbound` resolving the trunk behind an inbound INVITE: a lone trunk on the socket (exact and host-only) is unique; trunks sharing a socket are told apart by Request-URI user (Contact user), then To AOR, then To user; an exact port match beats a host-only one; an undecidable shared socket still returns a trunk but reports it as not unique, so the leg does not inherit that trunk's `app_id`; unknown and empty hosts return nil |
| `internal/sip` (registrar app ownership) | 3 | A refresh without `app_id` keeps the binding's owner and every `sip.registration_active` carries it; `AppIDFor` resolves by exact contact, falls back to the app shared by all of the AOR's bindings, and returns none when they disagree or the AOR is unknown; `LookupBySocket` returns every binding from a host:port and nothing for another port or an empty socket |
| `internal/sip` (client NAT) | 3 | Against a fake registrar: the REGISTER's Via carries `;rport` (RFC 3581) while its sent-by stays pinned to the advertised host:port — outbound requests leave from an ephemeral socket, so a registrar answering to sent-by rather than symmetrically black-holes the response and the trunk sits out Timer_B. Plus `registerOnce` reporting a digest-retry transport failure instead of dereferencing the nil response a timeout returns (a registrar that challenges then goes silent used to panic the process). Plus the `SIP_DEBUG` wire dump of an outbound REGISTER carrying the Via sipgo actually sent — the dump was taken before `client.Do`, which is where the Via is added, so every logged REGISTER was Via-less and `;rport` was invisible in the logs someone reads to diagnose a registration failure |
...
| `internal/api` (agent playout buffer) | 5 | `streamBuffer` serves two consumers with different clocking needs: `Read` keeps its frame-interval pacing for the room path, where the mixer's lossy incoming channel drops what arrives early, while `ReadUnpaced` — the leg-agent path, whose sink already blocks for backpressure — returns buffered frames as fast as they arrive, closes with `io.EOF` (partial frame first) and accepts an empty slice. The leg shape is asserted end to end: reading unpaced saturates the bounded sink that `sip_leg`'s writeLoop drains, giving a late reader a full cushion of frames to be late against, which a reader capped at the consumer's own rate can never build |
...
| `internal/leg` (codec renegotiation) | 6 | Mid-call re-INVITE onto a different codec: the encoder, decoder, framing and jitter-buffer geometry follow what the answer accepted (before, the leg answered "yes, G.722" and carried on decoding PCMU), and the room is told the rate moved; a re-offer of the running codec changes nothing and reports no rate change, which is what hold, unhold and SBC re-anchors send; an AMR-WB `mode-set` narrowing rebuilds the encoder without moving the rate, so nothing downstream is touched; queued old-rate PCM is dropped on a rate change; and the frame channels and RTP socket survive, because the room's reader is bound to the one and the peer already has the other in its SDP |
...
| `internal/sip` (multi-stream SDP) | 26 | `ParseSDP` keeps every `m=audio` section and every `m=` line of any kind (`Audio[]`/`MLines[]`/`PrimaryAudio`), picks the first non-rejected section as primary, errors only when all audio is rejected, parses `a=mid`/`a=label`/`a=content`/`a=lang`/`a=rtcp-mux`, inherits session-level direction and lets media-level override it; legacy scalars stay in sync with the primary stream; `AudioByMID`, `MirrorDirection` (RFC 3264 §6.1 pairing), `HoldDirection`; `AudioIsSRTPOnly` flags an offer whose every active audio section is SRTP (SDES `RTP/SAVP`, DTLS `UDP/TLS/RTP/SAVPF`) and clears one that still has a plain-RTP section, counting a port-0 section as inactive |
...
| `TestCustomData_OutboundLegEvents` | `custom_data` supplied on `POST /v1/legs` is echoed on the create response and `GET /v1/legs/{id}`, and rides `leg.ringing`, `leg.connected` and `leg.disconnected` over VSI — including the disconnect, which is published after the leg has left the manager |
| `TestCustomData_InboundAttachedAtAnswer` | An inbound leg's `leg.ringing` carries no `custom_data`; attaching it on `POST /v1/legs/{id}/answer` makes it appear on `leg.connected` and on the leg view |
| `TestCustomData_OversizeRejected` | `custom_data` beyond `CUSTOM_DATA_MAX_BYTES` is rejected with `400` naming the limit, before any leg is created |
| `TestCustomData_VSICreateLeg` | VSI `create_leg` accepts `custom_data` and returns it on the result view; an oversize value comes back as an `error` frame, matching the REST behaviour |
| `TestCustomData_ModifyMidCall` | `PUT /v1/legs/{id}/custom-data` attaches and then replaces `custom_data` on a live call; each change shows up on the next event for that leg and the latest value rides `leg.disconnected` |
| `TestCustomData_DeleteEndpoint` | `DELETE /v1/legs/{id}/custom-data` clears the value; the returned leg view and every subsequent event omit the field |
| `TestCustomData_VSISetAndDelete` | VSI `set_leg_custom_data` / `delete_leg_custom_data` mirror the two REST routes and return the updated leg view; an unknown leg comes back as an `error` frame |
...
| `TestRoomCreate_ComfortNoiseOverride` | `POST /v1/rooms` with `comfort_noise` omitted/`false`/`true` sets the room mixer accordingly |
| `TestRoomCreate_ComfortNoiseOverridesServerDefault` | With the server default off, an omitted `comfort_noise` inherits it and `comfort_noise: true` turns it on for that room only |
| `TestVSI_CreateRoom_ComfortNoiseOff` | VSI `create_room` with `comfort_noise: false` disables comfort noise for that room |
...
| `TestCodecRenegotiation_MidCallRateChange` | A real dialog renegotiated mid-call: VoiceBlender calls a raw UA, the UA answers PCMU (8 kHz) then re-INVITEs to G.722 (16 kHz). Asserts the answer accepts G.722 **and** that the leg is actually running it — the stream view reports the live codec, so this fails if the pipeline keeps decoding PCMU behind an accepted answer |
| `TestAudioFilters_ServerDefault` | `AUDIO_FILTERS` reaches both legs of a call and the leg view reports it with its parameters |
| `TestAudioFilters_PerLegOverride` | A leg's own `filters` replaces the server default, and an explicitly empty array opts out rather than falling back to the default |
| `TestAudioFilters_RejectsBadChain` | Unknown filter, over-long chain, duplicate single-use filter and out-of-range parameter are all rejected with 400 at the request, not when audio starts |
| `TestAudioFilters_DenoiseDegrades` | With no denoise kernel installed, requesting `denoise` still places the call and the leg view reports no filtering |
| `TestAudioFilters_ChangeMidCall` | `PUT /v1/legs/{id}/filters` swaps a live leg's chain (`robotic` -> `pitch`), clears it to none, and enables then disables `denoise` — all while the call stays up |
| `TestAudioFilters_EnableOnUnfilteredLeg` | Denoise turned on mid-call for a leg created with `"filters": []`, the live-calls-board case: the room used to register no chain for an unfiltered leg, so there was nothing to change |
| `TestAudioFilters_DenoiseActive` | A denoising leg in a room holds a live pool state, and deleting the leg returns it — guarding against the state leak that would walk an instance toward its heap ceiling |
...
| `TestSIPRegister_AcceptClaimsAppID` | Accepting the first REGISTER with `app_id: acme` claims the binding: `sip.registration_active` carries `acme`, the first attempt is unscoped while the refresh attempt carries `acme`, a refresh accepted without `app_id` keeps the owner, `GET /v1/sip/registrations` shows it, and an INVITE from the registered socket rings with `app_id: acme` despite a spoofed `X-App-ID` |
| `TestSIPRegister_ChallengeClaimsAppID` | `app_id` given on a challenge survives to the binding made by the credentialed re-REGISTER (which is not consulted again) and to its `sip.registration_active` event |
...
| `TestTrunk_SIPRegister_ViaOffersRport` | The REGISTER the fake registrar receives from a wired-up trunk carries `;rport` on its Via, so a NATed deployment gets the response back on the socket it actually sent from |
...
| `TestTrunk_SIPRegister_InboundInheritsAppID` | Two trunks with different `app_id`s registered against the same fake registrar; an INVITE from that host addressed to each trunk's Contact user rings with that trunk's `trunk_id` and `app_id`, and the trunk's `app_id` beats a spoofed `X-App-ID` header |
...
| `TestTrunk_OutboundProxy_RegisterRoutedViaProxy` | Trunk whose `registrar_uri` points at a port nothing listens on and whose `outbound_proxy` points at the fake: the REGISTER arrives at the proxy — so the proxy socket really was the transport destination — carrying **no** `Route` header while its Request-URI still names the unreachable registrar, and the trunk reaches `active`. A pre-loaded Route is deliberately absent: a proxy that does not recognise the Route URI as one of its own forwards the REGISTER back at itself rather than popping it, and the registration times out with no response. `GET /v1/sip/trunks/{id}` reports the proxy |
