...
| `internal/api` (stt turn events) | 12 | Provider selection (`deepgram_flux` resolves to the Flux transcriber and shares `DEEPGRAM_API_KEY` with `deepgram`; `speechmatics` resolves to the Speechmatics transcriber against `SPEECHMATICS_API_KEY`); `STTRequest`→`stt.Options` mapping including pointer semantics (`endpointing: 0` is not the same as absent) and the 400s for out-of-range thresholds; `attachSTTSinks` publishing `stt.text` with `speech_final` and `stt.turn` with seconds converted to milliseconds; and the cross-wiring guard — a room shares one `Options` template across its legs, so the callbacks must bind to a per-leg copy or every leg's events would go to whichever started last |
