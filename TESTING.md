...
| `internal/leg` (answer negotiation) | 6 | `negotiateInboundAnswer` answers a two-`m=audio` offer with two sections — both accepted when multi-stream is on (distinct ports, `a=recvonly` mirroring, `a=mid`/`a=content`/`a=lang` echoed), the extra one port-0 rejected when off or past the cap; single-stream offers answer exactly as before with no unsolicited `a=mid`; no common codec fails; video is port-0 rejected under `SIP_SDP_STRICT_MLINE_ANSWER` |
| `internal/leg` (re-offer) | 6 | `ApplyRemoteOffer` re-applies the peer's media address on a re-INVITE (the regression guard for one-way audio after an SBC re-anchor), mirrors the offered direction into the answer, no-ops on a bodyless session-timer refresh, still answers a malformed offer; `applyAnswer` re-applies the address from a 200 OK to an offer we sent; `matchRemoteStream` prefers `a=mid` over m-line position |
...
| `internal/sip` (answer planning) | 11 | `PlanAnswer` emits one plan per offered `m=` section in order; rejects extras with `multi_stream_disabled` / `max_streams`, echoes peer-disabled sections, rejects `no_common_codec` per section, omits vs. port-0-rejects video depending on `StrictMLines`, skips the RTT section, applies the preferred codec only to the first stream, mirrors offered directions |
...
| `TestMultiStream_PeerRejectsExtraStreamCallSurvives` | B has multi-stream disabled, so it answers the extra section with port 0 while keeping the same m-line count; the call stays up on the primary stream alone |
...
| `TestMultiStream_RESTCreateWithStreamsValidation` | `POST /v1/legs` rejects an unsatisfiable `streams` request up front rather than as a failed INVITE: 409 with the gate off, 409 past `SIP_MULTI_STREAM_MAX`, 400 for an invalid direction, 404 for an unknown `room_id` |
...
| `TestMultiStream_RESTAddRejectedWhenDisabled` | `POST /v1/legs/{id}/streams` returns 409 when `SIP_MULTI_STREAM_ENABLED` is off |
...
| `TestMultiStream_DisabledEmitsSingleMLine` | With the default configuration a plain call still negotiates exactly one `m=audio` section |
| `TestMultiStream_RejectedWhenDisabledOnOfferer` | Offering multiple streams with `SIP_MULTI_STREAM_ENABLED=false` fails with an error naming the gate |
