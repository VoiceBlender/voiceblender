...
| `internal/room` (audio filters) | 3 | Per-leg chain selection: a leg that chose nothing takes the server default, a leg that chose one keeps it, and an explicitly empty chain is *not* given the default back (the opt-out would otherwise be silently undone); and a chain that cannot be built falls back to plain resampling so the leg keeps its audio; and a leg moved between rooms rebuilds its chain in the new room, hands the old chain's denoise state back to the pool, and keeps its own audio source open — the mixer closes a participant's reader on teardown, so a chain that propagated Close would leave a pipe-backed leg (a LiveKit participant) silent after the move |
...
| `TestAMD_NoisyAccuracy_Denoised` | The same corpora scored twice, unfiltered and through the push-mode chain the live AMD feed uses. Measured 1.1% correct as-is against 98.4% denoised over 184 files, with AMD already failing at 15 dB SNR — the evidence that AMD needs denoised audio on noisy trunks |
...
| `TestAudioFilters_ChangeMidCall` | `PUT /v1/legs/{id}/filters` swaps a live leg's chain (`robotic` -> `pitch`), clears it to none, and enables then disables `denoise` — which crosses the 16 kHz/48 kHz working-rate boundary and so rebuilds the chain behind a fade — all while the call stays up |
