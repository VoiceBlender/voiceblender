...
| `internal/sip` (client NAT) | 2 | Against a fake registrar: the REGISTER's Via carries `;rport` (RFC 3581) while its sent-by stays pinned to the advertised host:port — outbound requests leave from an ephemeral socket, so a registrar answering to sent-by rather than symmetrically black-holes the response and the trunk sits out Timer_B. Plus `registerOnce` reporting a digest-retry transport failure instead of dereferencing the nil response a timeout returns (a registrar that challenges then goes silent used to panic the process) |
