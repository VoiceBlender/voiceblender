...
| `COMFORT_NOISE_ENABLED` | `true` | Inject low-level comfort noise (~−75 dBFS) into otherwise silent mixer frames. Rooms can override it with `comfort_noise` on `POST /v1/rooms`. |
| `AUDIO_FILTERS` | *(empty)* | Default ingress audio processing chain for new legs, applied to audio arriving from a leg before it reaches the room mixer. Ordered, comma-separated, with optional `:name=value` parameters — e.g. `denoise` or `bandpass:low_hz=300:high_hz=3400,denoise`. Available filters: `denoise` (background noise suppression), `bandpass` (`low_hz` default 300, `high_hz` default 3400), `gain` (`volume` −8 to 8, ~3 dB per step), `pitch` (shift the voice by semitones; `semitones`, `mix`), `robotic` (metallic voice effect; `pitch_hz`, `depth`, `mix`), `vocoder` (fully synthetic robot voice, harder to understand; `carrier_hz`, `bands`, `mix`). Empty means no processing. A per-leg `filters` field on `POST /v1/legs` or `POST /v1/legs/{id}/answer` overrides this; send `[]` there for no processing. An unparseable value is logged and ignored rather than failing startup. |
