...
func TestCreateSIPOutboundLegBadAMDParamsEndsRootSpan(t *testing.T) {
	...
	// An analysis window too short to reach any verdict at all: every call on
	// the leg could only ever end not_sure, so Validate rejects it. Defeating
	// one verdict alone is legal (a large greeting_duration is how a caller
	// disables the machine verdict), so it has to be all three.
	_, err := s.doCreateSIPOutboundLeg(CreateLegRequest{
		Type: "sip",
		To:   "sip:bob@127.0.0.1:5060",
		AMD:  &AMDParams{TotalAnalysisTime: 1},
	})
