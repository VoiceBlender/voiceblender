...
// TestStopLegResult_EmptyFileWhenNothingPublished is the wire-level half: a
// discarded capture must still carry a `file` key, empty, so clients that read
// the field unconditionally keep working and the response matches the event.
func TestStopLegResult_EmptyFileWhenNothingPublished(t *testing.T) {
	s := newTestServer(t)
	...
	file, present := decoded["file"]
	if !present {
		t.Errorf("response dropped the file key; it must stay present and empty: %s", body)
	}
	if file != "" {
		t.Errorf("file = %v, want empty — nothing was published: %s", file, body)
	}
}
