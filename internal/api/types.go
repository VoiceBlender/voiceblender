...
var sttRequestFields = map[string]FieldEnrichment{
	...
	"provider":            {Description: `STT provider: "elevenlabs" (default), "deepgram" (/v1/listen), "deepgram_flux" (/v2/listen, conversational turn detection), "azure" or "speechmatics"`, Enum: []string{"elevenlabs", "deepgram", "deepgram_flux", "azure", "speechmatics"}},
	"api_key":             {Description: "API key override (falls back to ELEVENLABS_API_KEY, DEEPGRAM_API_KEY, AZURE_SPEECH_KEY or SPEECHMATICS_API_KEY env var depending on provider)"},
	"model":               {Description: `Provider-specific model. Deepgram: default "nova-3". Deepgram Flux: "flux-general-en" (default) or "flux-general-multi". Speechmatics: "standard" (default) or "enhanced".`},
	"keyterms":            {Description: "Terms to boost recognition of (Deepgram, Deepgram Flux, and Speechmatics — where they become additional_vocab)."},
	"endpointing":         {Description: "Deepgram: milliseconds of silence before a segment is finalized; 0 disables endpointing. Speechmatics: maps to max_delay, clamped to 700-4000 ms; 0 leaves the provider default."},
	"utterance_end_ms":    {Description: "Deepgram: milliseconds of silence after which an stt.turn event with event=utterance_end is emitted. Deepgram requires interim results for this, which are requested automatically and still suppressed unless partial is true. Speechmatics: milliseconds of silence that close a turn and emit an stt.turn event with event=end_of_turn — default 600, capped at 2000, and 0 disables turn detection."},
