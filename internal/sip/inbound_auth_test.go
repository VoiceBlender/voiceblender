...
func TestRecordChallenge_HeaderShape(t *testing.T) {
	...
	val := e.recordChallenge("call-1", ChallengeParams{Realm: "vb.example", Password: "s3cret"})
...
func TestRecordChallenge_NonceUnique(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		val := e.recordChallenge("call", ChallengeParams{Realm: "r"})
...
func TestVerifyInboundAuth_ValidPassword(t *testing.T) {
	for _, alg := range []string{"MD5", "SHA-256"} {
		t.Run(alg, func(t *testing.T) {
			e := newAuthTestEngine(time.Minute)
			val := e.recordChallenge("c1", ChallengeParams{Realm: "vb", Username: "alice", Password: "pw", Algorithm: alg})
			req := authRequest(t, sip.INVITE, "c1", val, "alice", "pw", "", "sip:vb@vb.example")
			res, user := e.VerifyInboundAuth(req, "INVITE")
			if res != AuthValid {
...
func TestVerifyInboundAuth_ValidHA1(t *testing.T) {
	...
	val := e.recordChallenge("c1", ChallengeParams{Realm: "vb", Username: "alice", HA1: ha1})
	req := authRequest(t, sip.REGISTER, "c1", val, "alice", "", ha1, "sip:vb.example")
	if res, _ := e.VerifyInboundAuth(req, "REGISTER"); res != AuthValid {
		t.Fatalf("result = %v, want AuthValid", res)
	}
}
...
func TestVerifyInboundAuth_WrongPassword(t *testing.T) {
	e := newAuthTestEngine(time.Minute)
	val := e.recordChallenge("c1", ChallengeParams{Realm: "vb", Username: "alice", Password: "right"})
	// Client signs with the wrong password.
	req := authRequest(t, sip.INVITE, "c1", val, "alice", "wrong", "", "sip:vb@vb.example")
	if res, _ := e.VerifyInboundAuth(req, "INVITE"); res != AuthInvalid {
...
func TestVerifyInboundAuth_WrongUsername(t *testing.T) {
	e := newAuthTestEngine(time.Minute)
	val := e.recordChallenge("c1", ChallengeParams{Realm: "vb", Username: "alice", Password: "pw"})
...
func TestVerifyInboundAuth_NoChallengeRecorded(t *testing.T) {
	...
	val := other.recordChallenge("c1", ChallengeParams{Realm: "vb", Password: "pw"})
	req := authRequest(t, sip.INVITE, "c1", val, "alice", "pw", "", "sip:vb@vb.example")
	if res, _ := e.VerifyInboundAuth(req, "INVITE"); res != AuthNone {
...
func TestVerifyInboundAuth_SingleUse(t *testing.T) {
	...
	val := e.recordChallenge("c1", ChallengeParams{Realm: "vb", Username: "alice", Password: "pw"})
	req := authRequest(t, sip.INVITE, "c1", val, "alice", "pw", "", "sip:vb@vb.example")
	if res, _ := e.VerifyInboundAuth(req, "INVITE"); res != AuthValid {
	...
	if res, _ := e.VerifyInboundAuth(req2, "INVITE"); res != AuthNone {
