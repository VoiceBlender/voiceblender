...
// srcRate — out as mono PCM at targetRate. Split from streamMP3 so the frame
// loop can be driven from raw PCM in tests (no MP3 encoder is available, and
// silence fixtures cannot distinguish a per-frame filter from a persistent one).
...
// stream, or returns nil when the rates match (a nil resampler is a passthrough).
//
// The filter is stateful, so its lifetime must be exactly one stream: build it
// once before the frame loop and reuse it for every frame (per-frame rebuilds
// re-zero the filter and gap every frame), but do not hang it on the long-lived
// Player (that would bleed one playback's tail into the next). A repeat playback
// builds a fresh filter per iteration by design — each iteration reseeks to the
// start of the file.
