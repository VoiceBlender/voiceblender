...
// TestStreamRawPCM_ResamplerIsPerStream is the opposite guard from the seam
// tests: a resampler hung on the long-lived Player would carry one playback's
// filter history into the next. A loud tone then silence on the same Player must
// come out silent; a per-Player filter would ring the tone on into the silence.
