...
const (
	// DefaultDeepgramFluxURL is the public Flux endpoint, used when
	// NewDeepgramFlux is given an empty base URL.
	DefaultDeepgramFluxURL = "wss://api.deepgram.com/v2/listen"
...
// NewDeepgramFlux builds a Flux transcriber against baseURL; empty means the
// public Deepgram endpoint.
func NewDeepgramFlux(log *slog.Logger, baseURL string) *FluxTranscriber {
	if baseURL == "" {
		baseURL = DefaultDeepgramFluxURL
	}
	return &FluxTranscriber{log: log, baseURL: baseURL}
}

func buildFluxURL(baseURL string, opts Options) string {
	model := opts.Model
	if model == "" {
...
func (t *FluxTranscriber) Start(ctx context.Context, reader io.Reader, apiKey string, opts Options, cb TranscriptCallback) error {
	...
	wsURL := buildFluxURL(t.baseURL, opts)
