...
func TestSlotBuffer_AppendAndBound(t *testing.T) {
	const slotBytes = 4
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			c := newSlotBuffer(slotBytes, tc.maxSlots)
...
func TestSlotBuffer_PopOnEmpty(t *testing.T) {
	c := newSlotBuffer(4, 4)
	slot, ok := c.pop()
	if ok {
...
func TestSlotBuffer_PopUnderflowKeepsPartialSlot(t *testing.T) {
	const slotBytes = 4
	c := newSlotBuffer(slotBytes, 4)
...
func TestSlotBuffer_PopAdvancesOneSlotAtATime(t *testing.T) {
	const slotBytes = 4
	c := newSlotBuffer(slotBytes, 8)
...
// TestSlotBuffer_AppendAcrossSlotBoundaries feeds byte runs that do not
// line up with slot boundaries, mirroring a writer whose frames are not exactly
// one slot each.
func TestSlotBuffer_AppendAcrossSlotBoundaries(t *testing.T) {
	const slotBytes = 4
	c := newSlotBuffer(slotBytes, 2)
