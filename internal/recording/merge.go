...
type MultiChannelResult struct {
	...
	// OmittedLegs names participants who took part but are absent from the merged
	// file, because their capture produced nothing to merge. MergeMultiChannel
	// never sets it — it merges exactly the inputs it is given; whoever selects
	// those inputs fills this in.
	OmittedLegs []string
}
...
func MergeMultiChannel(dir string, inputs []MultiChannelInput, totalDuration time.Duration, sampleRate int) (*MultiChannelResult, error) {
	...
	// Staged so a caller listing the directory never sees a half-written merge.
	...
	// Any return before the finalize step below is a failure and must leave
	// nothing behind at either name.
	finalized := false
	defer func() {
		if !finalized {
			enc.Close()
