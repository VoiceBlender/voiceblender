...
- **Audio filters** -- per-leg ingress processing chain applied before audio reaches the mixer, so other legs, recordings and STT all see the cleaned audio. Built-in filters: `denoise` (background noise suppression via RNNoise, running as WebAssembly under wazero -- no cgo, ~2% of a core per stream), `bandpass` and `gain`. Configure per leg with `filters` on `POST /v1/legs` or `/answer`, or set a default with `AUDIO_FILTERS`. Off by default. See [API.md](API.md#audio-filters).
