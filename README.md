...
| `SIP_TCP_ENABLED` | `false` | Listen for SIP over TCP on `SIP_PORT` alongside the UDP listener. UDP caps a SIP message at 1300 bytes (RFC 3261 §18.1.1), which a SIPREC INVITE always exceeds because it carries the recording metadata document alongside the SDP — so this must be on for `SIPREC_ENABLED` to be usable. Off by default so an existing deployment's listening surface is unchanged. |
