...
| `MIXER_SOLE_CLOCK` | `false` | When `true`, WS/agent ingress blocks for real PCM (no Sleep pace, no invented silence). Pair with a deeper live queue. |
| `MIXER_LIVE_QUEUE_DEPTH` | `3` | `AddParticipant` incoming/outgoing channel depth. |
| `COMFORT_NOISE_ENABLED` | `true` | Inject low-level comfort noise (~−75 dBFS) into silent mixer frames. |
