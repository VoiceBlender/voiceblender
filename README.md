...
| `SIP_OUTBOUND_REGISTRATION_FAILURE_BACKOFF_MAX_MS` | `300000` | Upper cap on the exponential backoff between failed outbound REGISTER attempts. Failures emit `sip.outbound_registration_failed`; the trunk stays in the manager and keeps retrying, unless the failures are credential rejections (see `SIP_OUTBOUND_REGISTRATION_AUTH_FAILURE_LIMIT`). |
| `SIP_OUTBOUND_REGISTRATION_AUTH_FAILURE_LIMIT` | `3` | How many consecutive credential rejections (`401`/`403`/`407` answered to a REGISTER that already carried digest credentials, or a `401`/`407` carrying no challenge to answer) end the refresh loop for good. The trunk then goes `terminated`: it emits `sip.outbound_registration_expired` (`reason: credentials_rejected`), stops matching `POST /v1/legs`, and stays readable via `GET /v1/sip/trunks/{id}` until deleted. Set to `0` to retry forever. Any success resets the counter. |
