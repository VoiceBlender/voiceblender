...
- **Multi-stream SIP calls** -- several `m=audio` sections in one dialog (RFC 3264), each with its own RTP port, direction, language and mixer room; built for live translation, where the original audio and a translated feed are mixed separately. Off by default (`SIP_MULTI_STREAM_ENABLED`); follows the SIPREC (RFC 7866) wire profile for interoperability. See [API.md](API.md#per-leg-audio-streams-multiple-maudio-lines).
...
| `SIP_MULTI_STREAM_ENABLED` | `false` | Allow a SIP dialog to negotiate more than one `m=audio` section (RFC 3264 §5.1), so one call can carry several independent audio streams — e.g. original plus translated audio, each routed to its own room. When `false`, extra audio sections a peer offers are answered with port 0 and the per-leg stream API refuses to add any. Off by default: many endpoints and SBCs assume exactly one `m=audio` line. |
| `SIP_MULTI_STREAM_MAX` | `4` | Maximum audio sections per SIP dialog when `SIP_MULTI_STREAM_ENABLED=true`. Each stream consumes one port from the `RTP_PORT_MIN`/`RTP_PORT_MAX` pool. Clamped to `1`..`16`. |
